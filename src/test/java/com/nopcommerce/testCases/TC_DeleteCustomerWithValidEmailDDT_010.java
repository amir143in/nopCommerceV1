...
public class TC_DeleteCustomerWithValidEmailDDT_010 extends BaseClass {
		...
		aa: do {
			...
			for (int i = 1; i <= entriesCount; i++) {
				...
				if (nameValue.equalsIgnoreCase(CustomerEmail)) {
					...
					addcust.clickOnDeleteButton();
					addcust.clickOnConfrimDeleteAlert();
					
					String msg = driver.findElement(By.tagName("Body")).getText();
					// System.out.println("The confirmation message is : " + msg);

					if (msg.contains("The customer has been deleted successfully.")) {
						Assert.assertTrue(true);
						logger.info("The Test case passed....!!!!");
					} else {
						captureScreen(driver, "AddNewCustomer");
						Assert.assertTrue("Customer didnt get delete", false);
					}
	...
	String[][] getData() throws IOException {
		...
		int rownum = XLUtils.getRowCount(path, "Hoja3");
		System.out.println("The number of rows count : " + rownum);
		int colcount = XLUtils.getCellCount(path, "Hoja3", 1);
		...
		for (int i = 1; i <= rownum; i++) {
			for (int j = 0; j < colcount; j++) {
				logindata[i - 1][j] = XLUtils.getCellData(path, "Hoja3", i, j);// 1 0
			}
